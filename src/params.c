...
char* par_names[n_pars] = {
    ...
    "интервал: интервал, который мы ждем прежде чем вернуть робота обратно (секунды)",
    "мячик: диаметр мячика (сантиметры)"
};
// Значения по умолчанию (на тот случай, если в файле нет данных для параметра)
int def_vals[n_pars] = { 7, 10, 60, 5000, 400, 2500, 10, 3, 23 };
